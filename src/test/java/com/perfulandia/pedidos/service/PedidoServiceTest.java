...
class PedidoServiceTest {
    ...
    @Test
    void testObtenerAlertasPorSucursalYProducto_ConResultados() {
        Long sucursalId = 1L;
        Long productoId = 2L;
        ...
        when(inventarioClient.listarAlertasPorSucursalYProducto(sucursalId, productoId))
                .thenReturn(List.of(alerta));

        List<AlertaInventario> resultado = pedidoService.obtenerAlertasPorSucursalYProducto(sucursalId, productoId);

        assertNotNull(resultado);
        assertEquals(1, resultado.size());
        assertEquals("Stock crítico", resultado.get(0).getMensaje());
        verify(inventarioClient).listarAlertasPorSucursalYProducto(sucursalId, productoId);
    }

    @Test
    void testObtenerAlertasPorSucursalYProducto_SinResultados() {
        Long sucursalId = 1L;
        Long productoId = 2L;
